...
Isolation Forest is a multivariate anomaly detection algorithm that identifies unusual observations based on their isolation in feature space. Unlike Cook's distance (which measures influence on regression coefficients), Isolation Forest detects data points that are inherently unusual regardless of their effect on the model.

#### 5.8.1 Raw Data Anomalies

We screen the raw velocity measurements for anomalies before modeling.

```{r tbl-raw-anomalies}
#| label: tbl-raw-anomalies
#| tbl-cap: "Raw data anomaly detection results using Isolation Forest"

if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  av <- lmm_results$advanced_validation

  if (!is.null(av$anomalies) && !is.null(av$anomalies$raw_data)) {
    raw <- av$anomalies$raw_data
    ...
    anomaly_df <- data.frame(
      ...
      Value = c(
        raw$n_total,
        raw$n_anomalies,
        sprintf("%.1f%%", raw$anomaly_rate * 100),
        sprintf("%.3f", raw$threshold),
        sprintf("%.3f", raw$score_mean),
        sprintf("%.3f", raw$score_max)
      )
    )

    format_table(anomaly_df, col.names = c("Metric", "Value"))
  }
}
```

**Interpretation**: Isolation Forest scores range from 0 (normal) to 1 (anomalous). The detected anomalies represent observations with unusual combinations of velocity, RIR, and load that warrant investigation.

```{r fig-anomaly-score-distribution}
#| label: fig-anomaly-score-distribution
#| fig-cap: "Distribution of Isolation Forest anomaly scores with threshold visualization. The red line indicates the threshold used for flagging observations. Dotted lines show the 90th, 95th, and 99th percentiles to contextualize the threshold choice."
#| fig-width: 10
#| fig-height: 5
...
if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  ...
  if (!is.null(av$anomalies) && !is.null(av$anomalies$raw_data_result)) {
    # Use the full result object for visualization
    anomaly_detector$plot_score_distribution_with_threshold(
      av$anomalies$raw_data_result,
      show_quantiles = TRUE,
...
The score distribution (@fig-anomaly-score-distribution) shows why the chosen threshold effectively separates anomalous from normal observations. The 5% contamination rate corresponds to the 95th percentile of scores, flagging only the most unusual observations.

#### 5.8.1.1 Anomaly Explanations: Feature Contributions

For each flagged observation, we calculate z-scores to identify which features contributed most to the anomaly classification.

```{r fig-anomaly-explanations}
#| label: fig-anomaly-explanations
#| fig-cap: "Feature contributions for flagged anomalies. Z-scores indicate how many standard deviations each observation deviates from the mean. Red indicates values higher than average (velocity faster than expected), blue indicates lower values (velocity slower than expected)."
#| fig-width: 10
#| fig-height: 6
...
if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  ...
  if (!is.null(av$anomalies) && !is.null(av$anomalies$raw_data_result)) {
    # Get the original data and features used
    data <- lmm_results$data
    features <- c("mean_velocity", "rir", "load_percentage_numeric")

    # Add numeric load percentage if not present
    if (!"load_percentage_numeric" %in% names(data)) {
      data$load_percentage_numeric <- as.numeric(gsub("%", "", data$load_percentage))
    }

    # Generate explanations
    explanation <- anomaly_detector$explain_anomalies(
      av$anomalies$raw_data_result,
      data,
      features,
      participant_col = "id"
    ...
    if (explanation$n_anomalies > 0) {
      anomaly_detector$plot_anomaly_explanations(
        explanation,
        title = "Feature Contributions to Anomaly Scores"
      )
    } else {
      cat("No anomalies detected to explain.")
    }
  }
}
```

```{r tbl-anomaly-explanations}
#| label: tbl-anomaly-explanations
#| tbl-cap: "Primary drivers for each flagged anomaly"
...
if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  ...
  if (!is.null(av$anomalies) && !is.null(av$anomalies$raw_data_result)) {
    ...
    if (explanation$n_anomalies > 0) {
      drivers <- explanation$get_primary_drivers()

      display_df <- data.frame(
        Participant = drivers$participant_id,
        `Anomaly Score` = sprintf("%.3f", drivers$anomaly_score),
        `Primary Driver` = drivers$primary_driver,
        Direction = drivers$primary_direction,
        check.names = FALSE
      )
...
The feature contributions (@fig-anomaly-explanations and @tbl-anomaly-explanations) reveal which specific measurements drove each anomaly. Observations flagged primarily for low velocity at high RIR may indicate fatigue or technique issues, while those with high velocity at low RIR may reflect unusually strong performance or measurement artifacts.

#### 5.8.2 Random Effects Anomalies
...
### 5.9 Comparison: Isolation Forest vs Cook's Distance {#sec-if-vs-cooks}

Cook's distance and Isolation Forest detect different types of unusual observations:

| Method | What It Detects | Strengths |
|--------|-----------------|-----------|
| **Cook's Distance** | Influential observations (affect regression coefficients) | Standard, interpretable |
| **Isolation Forest** | Multivariate anomalies (unusual in feature space) | No distributional assumptions |

```{r tbl-if-vs-cooks}
#| label: tbl-if-vs-cooks
#| tbl-cap: "Agreement between Isolation Forest and Cook's Distance for flagging anomalies"
...
if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  ...
  if (!is.null(av$if_vs_cooks_comparison)) {
    ...
    comparison_df <- data.frame(
      Category = c(
        "Both Methods Agree (Flagged)",
        "Only Isolation Forest Flags",
        "Only Cook's Distance Flags",
        "Neither Method Flags",
        "Overall Agreement Rate",
        "Jaccard Index"
      ),
      Value = c(
        comp$both_flagged,
        comp$only_isolation_forest,
        comp$only_cooks_distance,
        comp$neither_flagged,
        sprintf("%.1f%%", comp$agreement_rate * 100),
        sprintf("%.3f", comp$jaccard_index)
      )
    )

    format_table(comparison_df, col.names = c("Category", "Value"))
  }
}
```
...
if (!is.null(lmm_results) && !is.null(lmm_results$advanced_validation)) {
  ...
  if (!is.null(av$if_vs_cooks_comparison)) {
    ...
    cat("**Interpretation:** ", comp$interpretation, "\n\n")

    if (comp$agreement_rate > 0.8) {
      cat("High agreement between methods suggests consistent identification of unusual observations.\n")
    } else if (comp$agreement_rate > 0.5) {
      cat("Moderate agreement indicates the methods detect different aspects of data quality.\n")
    } else {
      cat("Low agreement confirms these methods are complementary rather than redundant. ",
          "Cook's distance focuses on regression influence, while Isolation Forest ",
          "detects multivariate outliers in the feature space.\n")
    }
  }
}
```

### 5.10 Model Calibration {#sec-calibration}
