...
```{r fig-hero-spaghetti}
#| label: fig-hero-spaghetti
#| fig-cap: "Individual Velocity-RIR Trajectories: Each gray line represents one athlete's unique velocity-RIR relationship. The red line shows the population average."
#| fig-height: 5
...
This visualization captures our central finding: while velocity decreases as athletes approach failure (the general trend), **each individual has a unique pattern**. Using population averages (red line) instead of individual calibration introduces substantial prediction error—this is why individual ~2x more accurate than general models.
...
```{r fig-diagnostics-panel}
#| label: fig-diagnostics-panel
#| fig-cap: "LMM Diagnostic Plots: Q-Q plot assesses residual normality; Residuals vs Fitted identifies heteroscedasticity patterns."
#| fig-height: 5
#| fig-width: 12

if (!is.null(lmm_results) && !is.null(lmm_results$diagnostics_full)) {
  diag_full <- lmm_results$diagnostics_full
  plot_diagnostics_panel(
    residuals = diag_full$residuals,
    fitted = diag_full$fitted
  )
}
```

The Q-Q plot shows how closely residuals follow a normal distribution (deviations at the tails are common and not problematic). The residuals vs fitted plot checks for heteroscedasticity—a flat, even spread is ideal. Any "funnel" pattern would suggest non-constant variance.
