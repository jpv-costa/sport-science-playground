...
**What This Tests:**
LMMs make assumptions about the residuals (normality, constant variance). We assess whether our data meets these assumptions.
...
if (!is.null(lmm_results) && !is.null(lmm_results$diagnostics)) {
  ...
  cat("| Test | Statistic | p-value | Interpretation |\n")
  cat("|------|-----------|---------|----------------|\n")

  # Normality test
  if (!is.null(diag$normality_test)) {
    norm <- diag$normality_test
    cat(sprintf("| %s (Normality) | W = %.4f | %s | %s |\n",
                norm$test_name,
                norm$statistic,
                format_p(norm$p_value),
                norm$interpretation))
  }
  ...
  if (!is.null(diag$homoscedasticity_test)) {
    ...
    cat(sprintf("| %s (Variance) | r = %.3f | %s | %s |\n",
                homo$test_name,
                homo$correlation,
                format_p(homo$p_value),
                ifelse(homo$is_homoscedastic, "Constant variance", "Potential heteroscedasticity")))
  }
  ...
  if (!is.null(diag$normality_test)) {
    if (diag$normality_test$is_normal) {
      cat("- ✓ Residuals appear normally distributed\n")
    } else {
      cat("- ⚠ Residuals show some departure from normality, but LMMs are robust to mild violations\n")
    }
  }

  # Homoscedasticity interpretation
  if (!is.null(diag$homoscedasticity_test)) {
    if (diag$homoscedasticity_test$is_homoscedastic) {
      cat("- ✓ Residual variance appears constant (homoscedastic)\n")
    } else {
      cat("- ⚠ Some heteroscedasticity detected (r =", round(diag$homoscedasticity_test$correlation, 3), ") - this is why we use robust standard errors\n")
    }
  }

  cat("\n**Note:** These diagnostic violations justify our use of cluster-robust standard errors and bootstrap confidence intervals, which do not rely on these assumptions.\n")
} else {
  cat("*Model diagnostics will be available after running the full analysis pipeline.*\n")
}
```
